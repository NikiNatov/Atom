...
namespace Atom
{
    ...
    public:
        ...
        inline IDXGIFactory7* GetDXGIFactory() const { return m_DXGIFactory; }
        inline IDXGIAdapter4* GetDXGIAdapter() const { return m_DXGIAdapter; }
        inline ID3D12Device6* GetD3DDevice() const { return m_D3DDevice; }
    private:
        DXGI_ADAPTER_DESC3                m_Description;
        D3D12_FEATURE_DATA_D3D12_OPTIONS5 m_FeatureOptions;
        IDXGIFactory7*                    m_DXGIFactory;
        IDXGIAdapter4*                    m_DXGIAdapter;
        ID3D12Device6*                    m_D3DDevice;
