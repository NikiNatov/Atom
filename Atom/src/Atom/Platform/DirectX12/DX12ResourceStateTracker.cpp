...
namespace Atom
{
    HashMap<ID3D12Resource*, DX12ResourceStateTracker::TrackedResourceState> DX12ResourceStateTracker::ms_GlobalStates;
    std::mutex DX12ResourceStateTracker::ms_GlobalMutex;

    // -----------------------------------------------------------------------------------------------------------------------------
    DX12ResourceStateTracker::DX12ResourceStateTracker(DX12CommandBuffer& commandBuffer)
    ...
    // -----------------------------------------------------------------------------------------------------------------------------
    void DX12ResourceStateTracker::AddTransition(ID3D12Resource* resource, D3D12_RESOURCE_STATES afterState, u32 subresourceIndex)
    {
    ...
    // -----------------------------------------------------------------------------------------------------------------------------
    void DX12ResourceStateTracker::CommitPendingBarriers()
    {
    ...
    // -----------------------------------------------------------------------------------------------------------------------------
    void DX12ResourceStateTracker::CommitBarriers()
    {
    ...
    // -----------------------------------------------------------------------------------------------------------------------------
    void DX12ResourceStateTracker::UpdateGlobalStates()
    {
        std::lock_guard<std::mutex> lock(ms_GlobalMutex);
    ...
    // -----------------------------------------------------------------------------------------------------------------------------
    void DX12ResourceStateTracker::ClearStates()
    {
        m_PendingBarriers.clear();
        m_ResourceBarriers.clear();
        m_ResourceStates.clear();
