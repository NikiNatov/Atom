...
void CSMain(uint3 ThreadID : SV_DispatchThreadID)
{
    ...
    g_CubeMapPrefilterParams.EnvMap.GetDimensions(outputWidth, outputHeight, outputDepth);
    ...
    g_CubeMapPrefilterParams.EnvMapUnfiltered.GetDimensions(0, inputWidth, inputHeight, inputLevels);
	...
	for (uint i = 0; i < NumSamples; ++i) 
	{
        ...
        float3 Lh = TangentToWorld(SampleGGX(u.x, u.y, g_CubeMapPrefilterParams.Roughness), N, S, T);
		...
		if (cosLi > 0.0) 
		{
            ...
            float pdf = NdfGGX(cosLh, g_CubeMapPrefilterParams.Roughness) * 0.25;
    ...
    g_CubeMapPrefilterParams.EnvMap[ThreadID] = float4(color, 1.0);
}